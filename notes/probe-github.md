...
| ✓ | 네이버 업종 | OK | 2,461B | 업종별 등락 |
| ✓ | 네이버 테마 | OK | 737B | 테마별 등락(국장 특유 신호) |
| ✓ | 네이버 일별시세 API | OK | 1,683B | 종목별 과거 시세·이동평균 |
| ✓ | DART 공시목록(list) | OK | 346B | 종목별 최근 공시 |
| ✓ | DART 내부자거래(elestock) | OK | 1,122,551B | 임원·주요주주 매매 (★ 동시매수/매도 신호) |
| ✓ | DART 5%룰(majorstock) | OK | 14,796B | 대량보유 변동 (국민연금·블랙록 등) |
| ✓ | DART 고유번호 ZIP(corpCode) | OK | 3,593,585B | 종목코드→DART코드 매핑. 없으면 위 셋이 전부 죽는다 |
...
| | 소스 | 결과 | 크기 | 없으면 |
|---|---|---|---|---|
| ✓ | 나스닥 스크리너 | OK | 2,204,815B | 미국 전종목 7,113개 · 동적 선별 전체 |
| ✓ | 나스닥 실적 캘린더 | OK | 108,783B | 실적 발표 일정 |
...
| | 소스 | 결과 | 크기 | 없으면 |
|---|---|---|---|---|
| ✓ | 야후 파이낸스 차트 | OK | 1,612B | 지수·원자재·환율·금리 전부(매크로의 심장) |
| ✓ | 야후 티커 RSS | OK | 14,226B | 종목별 뉴스 |
...
| ✓ | ECB 독일 10년물 | OK | 1,560B | 매크로데스크 rateDiff 팩터 (미-유로존 금리차) |
| **✗** | 독일 국채 (야후 대체) | HTTP 404 | 108B | ECB가 막힐 때의 2차 대안 |
| ✓ | CNN 공포탐욕 | OK | 177,591B | 미 증시 심리 지표 |
...
| ✓ | 코인게코 | OK | 3,663B | 시총·도미넌스·상위 100 코인 |
...
| ✓ | OKX 상위트레이더 롱숏비 | OK | 3,897B | 상위계좌 롱숏비 대체 (소매-고래 분화) |
| ✓ | OKX 테이커 매수/매도 | OK | 27,831B | 테이커 비율 대체 |
| ✓ | OKX 티커 전체 | OK | 129,545B | 코인별 24시간 등락 |
...
| | 소스 | 결과 | 크기 | 없으면 |
|---|---|---|---|---|
| ✓ | 구글뉴스 RSS(한국어) | OK | 110,685B | 국장·코인 뉴스 수집의 주력 |
| ✓ | 연합뉴스 RSS | OK | 90,798B | 국내 경제 뉴스 |
| **✗** | 머니투데이 RSS | 연결실패(ConnectTimeout) | 0B | 국내 증권 뉴스 |
| ✓ | 매일경제 RSS | OK | 37,603B | 국내 증권 뉴스 |
| ✓ | 조선비즈 RSS | OK | 753B | 국내 경제 뉴스 |
| ✓ | 블록미디어 RSS | OK | 19,463B | 국내 코인 뉴스 |
| ✓ | 코인데스크 RSS | OK | 30,856B | 해외 코인 뉴스 |
| ✓ | CNBC RSS | OK | 20,675B | 미국 시장 뉴스 |
| ✓ | 오일프라이스 RSS | OK | 16,186B | 에너지 뉴스 |
