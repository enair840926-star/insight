...
| **✗** | 야후 금 현물 XAUUSD=X | HTTP 404 | 108B | 금을 선물(GC=F) 대신 현물로 받기 위한 1순위 후보 |
| **✗** | 야후 금 현물 XAU=X (대안) | HTTP 404 | 108B | XAUUSD=X가 안 되면 쓸 2순위 |
...
| ✓ | OKX 테이커 매수/매도 | OK | 27,706B | 테이커 비율 대체 |
| ✓ | OKX 티커 전체 | OK | 134,102B | 코인별 24시간 등락 |
...
| | 소스 | 결과 | 크기 | 없으면 |
|---|---|---|---|---|
| ✓ | 구글뉴스 RSS(한국어) | OK | 114,990B | 국장·코인 뉴스 수집의 주력 |
...
- **야후 금 현물 XAUUSD=X** — HTTP 404
  - 잃는 것: 금을 선물(GC=F) 대신 현물로 받기 위한 1순위 후보
  - 응답: `{"chart":{"result":null,"error":{"code":"Not Found","description":"No data found, symbol may be delisted"}}}`
- **야후 금 현물 XAU=X (대안)** — HTTP 404
  - 잃는 것: XAUUSD=X가 안 되면 쓸 2순위
  - 응답: `{"chart":{"result":null,"error":{"code":"Not Found","description":"No data found, symbol may be delisted"}}}`
