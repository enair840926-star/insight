...
위 데이터를 근거로 암호화폐 시장을 분석하라.

**가격 등락률 나열은 하지 마라.** 이 데이터의 가치는 가격이 아니라
포지셔닝에 있다. 아래를 우선하라.

1. **가격과 포지션의 관계** — 가격이 오르는데 미결제약정도 늘면 신규 자금
   유입이고, 가격은 오르는데 미결제약정이 줄면 숏 커버링이다. 둘은 전혀
   다른 의미다. 각 코인에서 어느 쪽인지 판단하고 근거를 대라.
2. **펀딩비 해석** — 극단적으로 높은 코인은 롱이 과밀하다는 뜻이고,
   그 상태에서 가격이 빠지면 연쇄 청산이 일어난다. 위험 구간에 있는
   코인을 지목하라. 시장 전체 평균 펀딩비와 양(+) 비율도 함께 보라.
3. **심리와 포지션의 괴리** — 공포탐욕 지수가 낮은데 롱 비율이 높다면
   "심리는 얼었는데 포지션은 롱에 몰려 있다"는 뜻이다. 이런 괴리를 찾아라.
   괴리가 없으면 "없음"이라고 명시하라.
4. **김치프리미엄** — 플러스면 국내가 비싸게 사는 중, 마이너스면 국내
   이탈이다. 절대값이 3%를 넘으면 이상 신호로 다뤄라. 코인별로 다르면
   그 차이가 무엇을 뜻하는지 해석하라.
5. **도미넌스** — BTC 도미넌스 방향이 알트코인 자금 흐름을 시사한다.
6. **관점별 정리** — 당일 / 단기(1~4주) / 장기(3개월+)
7. **주의 신호** — 청산 위험, 과열 구간, 데이터상 경계할 부분

각 주장에 구체적 숫자를 인용하라. 데이터에 없는 내용은 추측하지 말고
"데이터 없음"이라고 명시하라. 투자 권유가 아닌 정보 정리로 작성하라.""")
    return "\n".join(L)
