...
1. **오늘의 핵심** — 시장을 움직인 요인 3가지. 지수 등락률만으로는 부족하다.
   시장 폭(상승/하락 종목 수)과 시총가중 등락률을 대조하라. 둘이 어긋나면
   소수 대형주가 지수를 끌고 갔다는 뜻이고, 그건 지수만 보면 안 보인다.
2. **섹터 로테이션** — 어느 섹터에서 어느 섹터로 자금이 갔는가.
   거래대금과 시총가중 등락률을 함께 보라.
3. **개별 종목** — 선별 사유별로 다르게 다뤄라.
   · '급등'/'급락'은 무슨 일이 있었는지 뉴스에서 근거를 찾아라. 못 찾으면
     "뉴스에 근거 없음"이라고 명시하라 — 그것도 정보다.
   · '거래대금상위'는 방향보다 자금 집중 자체가 신호다.
   · '관심종목'은 조용해도 상태를 확인해 보고하라.
4. **관점별 정리** — 당일 / 단기(1~4주) / 장기(3개월+).
   단기에는 위 실적·경제지표 캘린더의 예정 이벤트를 반영하라.
5. **주의 신호** — 데이터상 경계할 부분.
