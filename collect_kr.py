...
from core import rules, env, screen
...
위 데이터를 근거로 분석하라. 규칙 기반 판정([긍정]/[부정])은 단순 키워드
매칭 결과이므로 참고만 하고, 숫자와 정면으로 배치되면 숫자를 따르라.

**뉴스의 날짜를 반드시 확인하라.** 며칠 전 기사가 섞여 있다. 과거 기사의
내용을 오늘 일어난 일로 서술하지 마라.

1. **오늘의 핵심** — 시장을 움직인 요인 3가지. 뉴스가 아니라 수급/매크로
   숫자에서 근거를 찾을 것. 지수 등락률만 보지 말고 시장 폭(상승/하락 종목
   수)과 시총가중 등락률을 대조하라. 둘이 어긋나면 소수 대형주가 지수를
   끌고 갔다는 뜻이고, 그건 지수만 보면 안 보인다.
   업종·테마 등락을 함께 보고 자금이 어디로 갔는지 짚어라.
2. **수급 해석** — 외국인/기관 방향이 뉴스 논조와 일치하는가, 배치되는가.
   배치된다면 그게 무엇을 뜻하는가. 내부자 거래가 있다면 외국인·기관
   방향과 같은지 다른지 비교하라. 단, 임원 개인의 소량 매매는
   스톡옵션 행사나 우리사주일 수 있으니 단독으로 해석하지 말고
   여러 명이 같은 날 같은 방향으로 움직인 경우만 신호로 취급하라.
3. **관점별 정리**
   - 당일: 오늘 확인된 사실과 그 의미
   - 단기(1~4주): 확인해야 할 변수, 예정된 이벤트
   - 장기(3개월+): 구조적 요인
4. **주의 신호** — 데이터상 경계할 부분

각 주장에 위 데이터의 구체적 숫자를 인용하라. 데이터에 없는 내용은
추측하지 말고 "데이터 없음"이라고 명시하라. 투자 권유가 아닌
정보 정리로 작성하라.""")
    return "\n".join(L)
