...
- 근거가 깨지는 지점: 블록의 '근거가 깨지는 지점'을 **그대로 옮겨라**
...
- **'근거가 깨지는 지점'은 손절선이 아니다.** 그 값이 무너지면 이 픽을
  세운 근거 하나가 사라진다는 뜻이지, 팔라는 신호가 아니다. "20일선
  106.84가 무너지면 '20일선 위' 근거가 사라집니다"까지 쓰고, 그때
  무엇을 할지는 읽는 분 몫으로 남겨라. 손절가·목표가·비중은 쓰지 않는다.
