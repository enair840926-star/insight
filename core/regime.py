...
def has_today_kr(b):
    ...
    개장 전 스냅샷은 지수 등락이 0.0이고 2,580개가 **전부 보합**으로 온다
    (`market_status` PREOPEN). 그대로 세면 상승비율 0%가 매일 '비우호'로
    잡히므로, 값이 있을 때만 국장 축을 켠다.

    저녁에 수집되면(마감 후) 값이 다 들어 있다. 그 경우까지 해외 값만 보면
    지수가 3.5% 급등한 날에도 판정 근거가 '미국 선물 +0.38%' 하나가 된다 —
    실측(08-13 22:16)에서 그렇게 나왔다.
    """
    br = _get(b, "aggregate", "breadth") or {}
    return bool(br.get("up") or br.get("down"))


def _kr(b):
    """국장. 오늘 값이 있으면 그것부터 세고, 없으면 해외 값으로만 잰다.

    축: (국장 방향 · 국장 폭) · 해외 위험선호 · 통화 · 금리 · 심리.
    선물과 VIX를 한 축에 둔 것은 둘이 같은 것을 재기 때문이다 — 선물이
    오른 날 VIX는 거의 항상 내린다.
    """
    mac = b.get("macro") or {}
    out = []
    ...
    if has_today_kr(b):
        ...
        out.append(_band(ag.get("market_cap_weighted_pct"), TH["cap_weighted"],
                         "국장 시총가중 등락"))
        # [축 0-2] 국장 폭 — 방향과 갈릴 수 있어 따로 둔다. 실측 08-13은
        # 시총가중 +3.41% 인데 상승비율 34.4% 였다(대형주만 오른 날).
        br = ag.get("breadth") or {}
        r = br.get("up_ratio")
        if r is not None and (r >= TH["breadth"] or r <= 100 - TH["breadth"]):
            out.append((1 if r >= TH["breadth"] else -1,
                        f"국장 상승비율 {r:.1f}% ({br.get('up', 0):,}/"
                        f"{br.get('counted', 0):,})"))
...
def _kr_basis(b):
    """국장 판정이 무엇에 근거하는가. 수집 시점에 따라 달라진다.

    한 문장으로 못박으면 틀린다 — 개장 전 문구를 마감 후 스냅샷에 그대로
    내보내면 "오늘 값은 아직 없다"고 하면서 지수가 3.56% 오른 날을 설명하게
    된다(실측 08-13 22:16).
    """
    if has_today_kr(b):
        return ("오늘 국장은 이미 마감됐다. 아래 국장 값은 그 결과이고, "
                "다음 장이 어떻게 열릴지는 해외 값 쪽이 더 크게 말한다.")
    return ("오늘 국장 값은 개장 전이라 아직 없다. 아래는 전부 밤사이 해외 "
            "값이므로 '어떻게 열릴까'이지 '장중에 어떨까'가 아니다.")


# 판정이 무엇에 근거하는가. 국장만 수집 시점에 따라 달라진다.
BASIS_FN = {"kr": _kr_basis}
...
def judge(market, bundle):
    ...
    return {
        ...
        "n": len(sigs),
        "basis": BASIS_FN[market](bundle) if market in BASIS_FN else None,
    }
