...
#
# **2026-08-16에 매크로 대상을 넷으로 늘렸다** (금·WTI원유 → +나스닥·유로달러).
# 매일 보는 지표인데 픽에 없으면 방향 판정이 남지 않아, 나중에 "그때 뭐라고
# 봤나"를 되짚을 수가 없다.
#
# **다만 이 둘은 근거가 얇다.** 재고도 선물 커브도 COT도 없어 시세만으로
# 재게 된다 — 실측(2026-08-14): 금 6개 · WTI원유 3개인데 나스닥 3개 ·
# 유로달러 2개다. 얇다는 사실은 `_caution`이 "근거가 N개뿐임"으로 화면에
# 남기므로 숨겨지지는 않는다. 근거를 두껍게 하려면 소스를 더 붙여야 하고,
# 그건 국장·미장을 먼저 보강한 뒤의 일이다(notes/going-public.md).
#
# `RULES_VERSION`은 올리지 않았다. 점수 규칙도 임계값도 그대로고 **대상만**
# 늘었다 — 올리면 국장·미장 기록까지 끊긴다(버전이 자산군별이 아니라 하나다).
# 매크로 집계를 예전과 이으려면 `key`로 금·WTI원유만 골라 내면 된다.
FOCUS = {
    "coin": ("BTC", "ETH"),
    "macro": ("금", "WTI원유", "나스닥", "유로달러"),
}
