...
위 데이터를 근거로 매크로 환경을 분석하라.

**개별 자산의 등락률 나열은 하지 마라.** 그건 어디서나 볼 수 있다.
이 데이터의 가치는 40개 자산을 나란히 놓았을 때만 보이는 것에 있다.

1. **오늘의 큰 그림** — 자금이 어디서 어디로 이동했는가. 위험선호인가
   회피인가, 아니면 어느 쪽도 아닌 혼재인가. 근거를 자산 간 상대 움직임에서 찾아라.
2. **금리 커브** — 스프레드의 부호와 방향이 무엇을 시사하는가.
   단기와 장기가 다른 방향이면 그 조합의 의미를 해석하라.
3. **깨진 관계** — [깨짐]으로 표시된 항목을 우선 다뤄라. 통상 관계가
   깨졌다면 (a) 일시적 노이즈인지 (b) 국면 전환 신호인지 판단하고 근거를 대라.
   깨진 관계가 없다면 "없음"이라고 명시하라.
4. **원자재 내부 분화** — 금속·에너지·농산물이 같은 방향인지 갈렸는지.
   갈렸다면 무엇이 그 차이를 만드는가.
5. **원자재 포지셔닝과 커브** — 가격만 보지 말고 아래 셋을 교차하라.
   · COT 투기 롱숏비가 극단인 품목을 지목하고, 그 방향이 최근 가격 움직임과
     같은 편인지 반대편인지 판단하라. 같은 편이면 과밀, 반대면 역행이다.
   · 순포지션의 **전주 대비 변화**를 보라. 절대 수준보다 방향 전환이 빠르다.
   · 선물 커브가 백워데이션인 품목은 현물이 타이트하다는 뜻이다. 가격이
     빠졌는데도 백워데이션이면 하락이 수급이 아니라 심리·지정학 때문일 수 있다.
   · ETF 거래량이 평균을 크게 웃도는데 가격은 조용하면 자금이 대기 중이라는 신호다.
   · **재고·커브·COT 세 축을 반드시 교차하라.** 재고가 평년 대비 타이트하고
     커브도 백워데이션이면 실물 수급 문제이므로 하락은 되돌려질 여지가 있다.
     반대로 재고는 평년 수준인데 커브만 백워데이션이면 지정학·심리 프리미엄이고,
     그 요인이 해소되면 커브가 먼저 무너진다. 어느 쪽인지 판단하고 근거를 대라.
6. **관점별 정리** — 당일 / 단기(1~4주) / 장기(3개월+)
7. **주의 신호** — 데이터상 경계할 부분

각 주장에 구체적 숫자를 인용하라. 데이터에 없는 내용은 추측하지 말고
"데이터 없음"이라고 명시하라. 규칙 기반 뉴스 판정은 단순 키워드 매칭이니
숫자와 배치되면 숫자를 따르라. 투자 권유가 아닌 정보 정리로 작성하라.""")
    return "\n".join(L)
